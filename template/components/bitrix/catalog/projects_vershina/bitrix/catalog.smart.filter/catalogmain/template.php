...
foreach ($arResult["ITEMS"] as $key=>$arItem)
    if (!empty($arItem['VALUES']))
        $arValues = array_merge($arValues, $arItem['VALUES']);

if (count($arValues) > 2) {
    $bDisplay = true;
} else if ($arValues['MIN']['VALUE'] != $arValues['MAX']['VALUE']) {
    $bDisplay = true;
}
				...
				<?foreach($arResult["ITEMS"] as $key=>$arItem)//prices
				{
					...
					if(isset($arItem["PRICE"])):
						...
						<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box bx-active">
							...
							<div class="bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)"><div class='bx-filter-parameters-box-hint'><?=str_replace("(продукция Таро)","(продукция Купикоробку.ру)",$arItem["NAME"])?> <i data-role="prop_angle" class="right fa fa-angle-<?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>up<?else:?>down<?endif?>"></i></div></div>
						...
						<div class="bx-filter-block" data-role="bx_filter_block">
							...
							switch ($arItem["DISPLAY_TYPE"])
							{
								...
								case "K"://RADIO_BUTTONS
									?>
									<div class="radio">
										<label class="bx-filter-param-label uni-button-radio" for="<?="all_".$arCur["CONTROL_ID"]?>">
												<input
													type="radio"
													value=""
													name="<?=$arCur["CONTROL_NAME_ALT"]?>"
												...
												<div class="bx-filter-param-text text"><?=GetMessage("CT_BCSF_FILTER_ALL");?></div>
											</span>
										</label>
									</div>
									<?foreach($arItem["VALUES"] as $val => $ar):?>
								...
								default: //CHECKBOXES
									...
									<?foreach($arItem["VALUES"] as $val => $ar):?>
										<div class="checkbox">
											<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label uni-button-checkbox <?=$ar["DISABLED"] ? 'disabled': '' ?>" for="<?=$ar["CONTROL_ID"] ?>">
												<input
													type="checkbox"
													value="<?=$ar["HTML_VALUE"] ?>"
													name="<?=$ar["CONTROL_NAME"] ?>"
